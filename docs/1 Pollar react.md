...
<KycModal onClose={() => setOpen(false)} country="US" level="basic" onApproved={() => console.log('KYC approved')} />;
```
...
<PollarProvider config={{ apiKey: '...' }} adapters={{ escrow: trustlessWorkEscrow }}>
  <App />
</PollarProvider>;
```
